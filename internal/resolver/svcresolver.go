...
func NewService(client kubernetes.Interface, enabled bool) *ServiceResolver {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "resolver(svc): List pods failed: %v\n", err)
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "resolver(svc): List Services failed: %v\n", err)
	} else {
		for i := range svcs.Items {
			r.addService(&svcs.Items[i])
		}
		fmt.Fprintf(os.Stderr, "resolver(svc): loaded %d services (%d svc IPs, %d pod IPs)\n",
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "resolver(svc): List EndpointSlices failed: %v\n", err)
	} else {
		for i := range eps.Items {
			r.addSlice(&eps.Items[i])
		}
		fmt.Fprintf(os.Stderr, "resolver(svc): loaded %d endpointslices (%d svc IPs, %d pod IPs)\n",
...
func (r *ServiceResolver) watchPods(client kubernetes.Interface) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "resolver(svc): Watch pods failed: %v\n", err)
		return
	}
	defer wi.Stop()
	...
	for {
		...
		case ev, ok := <-wi.ResultChan():
			if !ok {
				fmt.Fprintf(os.Stderr, "resolver(svc): Watch pods channel closed, restarting...\n")
				go r.watchPods(client)
				return
			}
...
func (r *ServiceResolver) watchServices(client kubernetes.Interface) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "resolver(svc): Watch Services failed: %v\n", err)
		return
	}
	defer wi.Stop()
	...
	for {
		...
		case ev, ok := <-wi.ResultChan():
			if !ok {
				fmt.Fprintf(os.Stderr, "resolver(svc): Watch Services channel closed, restarting...\n")
				go r.watchServices(client)
				return
			}
...
func (r *ServiceResolver) watchSlices(client kubernetes.Interface) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "resolver(svc): Watch EndpointSlices failed: %v\n", err)
		return
	}
	defer wi.Stop()
	...
	for {
		...
		case ev, ok := <-wi.ResultChan():
			if !ok {
				fmt.Fprintf(os.Stderr, "resolver(svc): Watch EndpointSlices channel closed, restarting...\n")
				go r.watchSlices(client)
				return
			}
